...
export default function Explore({ navigation }) {
  ...
  const filterMovie = (text) => {
    setSearchText(text)
    let filter = originalFilter.filter((item, index) => {
      return item.original_title.toLowerCase().includes(text.toLowerCase())
    })

    if (text == '') {
      setFilmData(originalFilter)
    } else {
      setFilmData(filter)  
    }
  }
    ...
    <View style={[styles.container]}>
      ...
      <View style={{ flexDirection: "row", alignItems: "center" }}>
        <View 
          ...
          <TextInput
            onChangeText={text => filterMovie(text)}
      ...
      <Text style={{ fontSize: 18, fontWeight: '900', color: '#000', marginTop: 15, alignSelf: 'flex-start', marginLeft: 15 }}>Top Search</Text>
      <FlatList
